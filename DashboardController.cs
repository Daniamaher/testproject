...
namespace CompanyProject.Controllers
{
    public class DashboardController :Controller
    {
        ...
        public DashboardController(IServiceRepository serviceRepository, INewsRepository newsRepository, IMediaRepository mediaRepository, IContactFormRepository contactFormRepository, UserManager<IdentityUser> userManager)
        {
            _serviceRepository = serviceRepository;
            _mediaRepository = mediaRepository;
            _userManager= userManager;
        ...
        public async Task<IActionResult> ContactFormIndex()
        {
            var contactForm = await _contactFormRepository.GetAllAsync();

           

            return View("ContactFormIndex", contactForm);
        }
        ...
        [HttpGet]
        public async Task<IActionResult> CreateContaxctForm()
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CreateContactForm(ContactForm contactForm)
        {
            

            await _contactFormRepository.AddAsync(contactForm);

            return RedirectToAction(nameof(ContactFormIndex));
        }
        ...
        [HttpGet]
        public async Task<IActionResult> EditContactForm(int id)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EditContactForm(ContactForm contactForm)
        {
            
            if (ModelState.IsValid)
            {
                var existingContactForm = await _contactFormRepository.GetByIdAsync(contactForm.Id);
                ...
                existingContactForm.Phone = contactForm.Phone;
                existingContactForm.IsActive = contactForm.IsActive;
                existingContactForm.IsDeleted = contactForm.IsDeleted;



                await _contactFormRepository.UpdateAsync(existingContactForm);
                return RedirectToAction(nameof(ContactFormIndex));
            }
        ...
        [HttpPost, ActionName("DeleteService")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeleteContactFormConfirmed(int id)
        {
        ...
        public async Task<IActionResult> NewsIndex()
        {
            ...
            var newsWithMedia = new List<(News news, Media? media)>();

            foreach (var newsItem in news) 
            {
                Media? media = null;
                if (newsItem.ImageGuid.HasValue)
                {
                    media = await _mediaRepository.GetMediaByGuidAsync(newsItem.ImageGuid.Value);
                }
                newsWithMedia.Add((newsItem, media)); 
            }

            return View(newsWithMedia);
        }
        ...
        public async Task<IActionResult> CreateNews(News news, IFormFile imageFile)
        {
            ...
            await _newsRepository.AddAsync(news, imageFile);
        ...
        {
            var service = await _newsRepository.GetByIdAsync(id);
            if (service == null)
            {
                return NotFound();
            }
        ...
        */
        ...
        public async Task<IActionResult> EditNews(News news, IFormFile? imageFile)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (imageFile != null && imageFile.Length > 0)
                {
                    if (existingNews.ImageGuid.HasValue)
                    {
                        await _mediaRepository.DeleteMediaAsync(existingNews.ImageGuid.Value);
                    }
                    ...
                    existingNews.ImageGuid = media.Id;
                }
        ...
        [HttpPost, ActionName("DeleteService")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeleteNwesConfirmed(int id)
        {
